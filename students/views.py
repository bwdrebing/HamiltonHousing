...
def building(request, building_name):
    ...
    rooms = list(Room.objects.all().filter(building=bldg).exclude(available=False).order_by('number'))
    return render(request, 
                  'students/building.html', 
                  {'current_building': bldg, 'building': bldg, 'rooms': rooms, 'buildings': building_list})